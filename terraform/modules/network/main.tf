...
resource "aws_security_group" "vault" {
  name        = "${var.project_name}-sg-vault"
  description = "Allow Vault API from EC2 only"
  vpc_id      = aws_vpc.main.id
  ...
  # Allow SSH from app servers (EC2 SG) so Ansible can jump through app1
  ingress {
    from_port       = 22
    to_port         = 22
    protocol        = "tcp"
  ...
  tags = { Name = "${var.project_name}-sg-vault" }
}
